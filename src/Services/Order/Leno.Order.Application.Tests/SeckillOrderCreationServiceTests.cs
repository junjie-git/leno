...
/// <summary>
/// 秒杀订单创建服务单元测试，验证消费 SeckillOrderCreatedEvent 后：
/// - SKU 有效时创建 OrderType.Seckill 订单并追加 SeckillOrderConfirmedEvent 回执（经 Outbox 同事务发布）；
/// - SKU 不存在或已下架时不创建订单，发布 SeckillOrderCreationFailedEvent 失败回执。
/// </summary>
