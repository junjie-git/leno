...
public sealed class StockReservation : AggregateRoot
{
    ...
    public static StockReservation Create(Guid id, Guid skuId, int baseLineQty)
    {
        if (skuId == Guid.Empty)
        {
            throw new OrderDomainException("SkuId 不可为空", "STOCK_SKU_EMPTY");
        }

        if (baseLineQty < 0)
        {
            throw new OrderDomainException("库存基线不可为负", "STOCK_BASELINE_INVALID");
        }
    ...
    public void ReserveStock(Guid orderId, int quantity)
    {
        ...
        {
            throw new OrderDomainException("OrderId 不可为空", "STOCK_ORDER_EMPTY");
        }

        if (quantity <= 0)
        {
            throw new OrderDomainException("预占数量须大于 0", "STOCK_RESERVE_QTY_INVALID");
        }
    ...
    public void ConfirmStockDeduction(Guid orderId, int quantity)
    {
        ...
        if (quantity <= 0)
        {
            throw new OrderDomainException("确认扣减数量须大于 0", "STOCK_CONFIRM_QTY_INVALID");
        }
    ...
    public void ReleaseStock(Guid orderId, int quantity)
    {
        ...
        if (quantity <= 0)
        {
            throw new OrderDomainException("释放数量须大于 0", "STOCK_RELEASE_QTY_INVALID");
        }
    ...
    public void Replenish(int delta)
    {
        if (delta == 0)
        {
            throw new OrderDomainException("库存基线增量不可为 0", "STOCK_REPLENISH_DELTA_ZERO");
        }
