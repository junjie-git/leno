...
/// 仅用于生成迁移与脚本，不连接真实数据库。
/// </summary>
public sealed class UserAuthDbContextDesignTimeFactory : IDesignTimeDbContextFactory<UserAuthDbContext>
{
    public UserAuthDbContext CreateDbContext(string[] args)
    {
        var options = new DbContextOptionsBuilder<UserAuthDbContext>()
            .UseSqlServer("Server=localhost,1433;Database=LenoUserAuth;User Id=sa;Password=Leno@SqlServer2019;TrustServerCertificate=True;MultipleActiveResultSets=true")
            .Options;
