...
public sealed class OAuthClient : AggregateRoot
{
        ...
        bool enabled = true)
    {
        if (id == Guid.Empty)
        {
            throw new UserAuthDomainException("OAuth 客户端标识不可为空", "OAUTH_CLIENT_ID_EMPTY");
        }
    ...
    private static void ValidateProvider(string provider)
    {
        ...
        {
            throw new UserAuthDomainException("OAuth2 提供方不可为空", "OAUTH_CLIENT_PROVIDER_EMPTY");
        }

        if (provider.Trim().Length is < 2 or > 32)
        {
            throw new UserAuthDomainException("OAuth2 提供方标识长度须为 2-32 字符", "OAUTH_CLIENT_PROVIDER_LENGTH");
        }
    }
    ...
    private static void ValidateClientId(string clientId)
    {
        ...
        {
            throw new UserAuthDomainException("ClientId 不可为空", "OAUTH_CLIENT_CLIENT_ID_EMPTY");
        }

        if (clientId.Trim().Length > 256)
        {
            throw new UserAuthDomainException("ClientId 长度不可超过 256 字符", "OAUTH_CLIENT_CLIENT_ID_LENGTH");
        }
    }
    ...
    private static void ValidateEncryptedSecret(string encryptedSecret)
    {
        ...
        {
            throw new UserAuthDomainException("ClientSecret 不可为空", "OAUTH_CLIENT_SECRET_EMPTY");
        }
    }
    ...
    private static void ValidateRedirectUri(string redirectUri)
    {
        ...
        {
            throw new UserAuthDomainException("RedirectUri 不可为空", "OAUTH_CLIENT_REDIRECT_URI_EMPTY");
        }

        if (!Uri.TryCreate(redirectUri, UriKind.Absolute, out var uri) ||
            (uri.Scheme != Uri.UriSchemeHttp && uri.Scheme != Uri.UriSchemeHttps))
        {
            throw new UserAuthDomainException("RedirectUri 必须为有效的 HTTP/HTTPS 地址", "OAUTH_CLIENT_REDIRECT_URI_FORMAT");
        }
    }
}