...
/// </summary>
public sealed class AfterSalesOrder : AggregateRoot
{
    ...
    private AfterSalesOrder() { }

    private AfterSalesOrder(Guid id) : base(id) { }
    ...
    public static AfterSalesOrder Create(
        Guid afterSalesId,
        Guid orderId,
        Guid? orderLineId,
        Guid userId,
        ...
        string currency)
    {
        ...
        {
            throw new AfterSalesDomainException("AfterSalesId 不可为空", "AFTERSALES_ID_EMPTY");
        }

        if (orderId == Guid.Empty)
        {
            throw new AfterSalesDomainException("OrderId 不可为空", "AFTERSALES_ORDER_EMPTY");
        }

        if (userId == Guid.Empty)
        {
            throw new AfterSalesDomainException("UserId 不可为空", "AFTERSALES_USER_EMPTY");
        }

        if (sellerId == Guid.Empty)
        {
            throw new AfterSalesDomainException("SellerId 不可为空", "AFTERSALES_SELLER_EMPTY");
        }

        if (!Enum.IsDefined(type))
        {
            throw new AfterSalesDomainException($"售后类型非法：{type}", "AFTERSALES_TYPE_INVALID");
        }

        if (string.IsNullOrWhiteSpace(reasonCategory))
        {
            throw new AfterSalesDomainException("原因分类不可为空", "AFTERSALES_REASON_CATEGORY_EMPTY");
        }

        if (string.IsNullOrWhiteSpace(reason))
        {
            throw new AfterSalesDomainException("申请原因不可为空", "AFTERSALES_REASON_EMPTY");
        }

        if (reason.Length > 500)
        {
            throw new AfterSalesDomainException("申请原因不可超过 500 字", "AFTERSALES_REASON_TOO_LONG");
        }

        var imageList = (images ?? []).ToList();
        if (imageList.Count > 5)
        {
            throw new AfterSalesDomainException($"凭证图片数量超限：{imageList.Count}，最多 5 张", "AFTERSALES_IMAGES_TOO_MANY");
        }

        if (requestedAmount <= 0)
        {
            throw new AfterSalesDomainException("申请金额须大于 0", "AFTERSALES_AMOUNT_INVALID");
        }

        var afterSales = new AfterSalesOrder(afterSalesId)
        {
            OrderId = orderId,
    ...
    public void Approve(Guid operatorId, decimal approvedAmount)
    {
        ...
        {
            throw new AfterSalesDomainException(
        ...
        {
            throw new AfterSalesDomainException("OperatorId 不可为空", "AFTERSALES_OPERATOR_EMPTY");
        }
    ...
    public void Reject(Guid operatorId, string reason)
    {
        ...
        if (string.IsNullOrWhiteSpace(reason))
        {
            throw new AfterSalesDomainException("驳回原因不可为空", "AFTERSALES_REJECT_REASON_EMPTY");
        }

        if (reason.Length > 200)
        {
            throw new AfterSalesDomainException("驳回原因不可超过 200 字", "AFTERSALES_REJECT_REASON_TOO_LONG");
        }
    ...
    public void ReturnGoods(string trackingNo)
    {
        ...
        {
            throw new AfterSalesDomainException("退货物流单号不可为空", "AFTERSALES_TRACKING_NO_EMPTY");
        }

        if (trackingNo.Length > 64)
        {
            throw new AfterSalesDomainException("退货物流单号不可超过 64 字", "AFTERSALES_TRACKING_NO_TOO_LONG");
        }
    ...
    public void MarkRefundCompleted(Guid refundId, decimal amount, string? channelRefundNo)
    {
        ...
        {
            throw new AfterSalesDomainException("RefundId 不可为空", "AFTERSALES_REFUND_ID_EMPTY");
        }
    ...
    public void MarkRefundFailed(string reason)
    {
        ...
        if (string.IsNullOrWhiteSpace(reason))
        {
            throw new AfterSalesDomainException("失败原因不可为空", "AFTERSALES_FAIL_REASON_EMPTY");
        }

        if (reason.Length > 512)
        {
            throw new AfterSalesDomainException("失败原因不可超过 512 字", "AFTERSALES_FAIL_REASON_TOO_LONG");
        }
    ...
    public void AddRefundRequestedEvent(Guid refundId, Guid paymentId, decimal refundAmount, string channel, string refundReason)
    {
        ...
        {
            throw new AfterSalesDomainException("PaymentId 不可为空", "AFTERSALES_PAYMENT_ID_EMPTY");
        }

        if (refundAmount <= 0)
        {
            throw new AfterSalesDomainException("退款金额须大于 0", "AFTERSALES_REFUND_AMOUNT_INVALID");
        }
    ...
    public void Cancel(Guid userId, string reason)
    {
        ...
        if (userId != UserId)
        {
            throw new AfterSalesDomainException("仅申请人可撤销售后单", "AFTERSALES_CANCEL_NOT_OWNER");
        }

        if (string.IsNullOrWhiteSpace(reason))
        {
            throw new AfterSalesDomainException("撤销原因不可为空", "AFTERSALES_CANCEL_REASON_EMPTY");
        }

        if (reason.Length > 200)
        {
            throw new AfterSalesDomainException("撤销原因不可超过 200 字", "AFTERSALES_CANCEL_REASON_TOO_LONG");
        }
