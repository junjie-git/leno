...
namespace Leno.AfterSales.Infrastructure.Configurations;

/// <summary>
/// AfterSalesOrder 售后单聚合根的 EF Core 映射配置（snake_case，售后 BC 独立维护）。
/// 凭证图片集合 _images 序列化为 JSON 列；售后类型与状态以 int 持久化；Version 作 rowversion 乐观锁。
/// 表名沿用 after_sales 保持数据迁移兼容性。
/// </summary>
public sealed class AfterSalesConfiguration : IEntityTypeConfiguration<AfterSalesOrder>
{
    public void Configure(EntityTypeBuilder<AfterSalesOrder> builder)
    {
        ...
        builder.Property(a => a.ReturnConfirmedBy).HasColumnName("return_confirmed_by");
