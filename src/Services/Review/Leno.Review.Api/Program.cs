...
using Leno.Review.Api.GrpcServices;
using Leno.Review.Infrastructure;
using Leno.Review.Infrastructure.Dependencies;
...
// 一站式注册：共享内核基础设施 + 鉴权 + 健康检查 + Controllers + OpenAPI + 评价域消费者 + 评价域基础设施
builder.Services.AddLenoApi<ReviewDbContext>(
    builder.Configuration,
    "leno-review-api",
    cfg => cfg.AddReviewConsumers(),
    s => s.AddReviewInfrastructure(builder.Configuration));
...
await app.Services.MigrateWithLockAsync<ReviewDbContext>();
app.Run();
