...
namespace Leno.Membership.Domain.Aggregates.Member;

/// <summary>
/// 会员聚合根，封装用户会员等级、累计消费、成长值与状态的不变量。
/// 聚合标识 <see cref="Entity.Id"/> 即对外 <c>MemberId</c>。
/// 成长值等级体系（V0-V4）由 <see cref="MemberLevel"/> 值对象定义，等级评估委托静态方法 <see cref="MemberLevel.EvaluateLevel"/>。
/// </summary>
public sealed class Member : AggregateRoot
{
    ...
    /// <summary>当前成长值（基于消费积分累计），用于 V0-V4 成长值等级体系。1 积分 = 1 成长值。</summary>
    ...
    public void AddConsumption(decimal amount)
    {
        if (amount <= 0)
        {
            throw new MembershipDomainException("消费金额须大于 0", "MEMBER_CONSUMPTION_INVALID");
        }
    ...
    /// <summary>
    /// 累加成长值，校验数量 &gt; 0。成长值在消费积分入账时同步增加（1 积分 = 1 成长值）。
    /// 将 reason 写入 <see cref="LevelChangeHistories"/> 子实体集合，
    ...
    /// <summary>
    /// 基于成长值评估等级（V0-V4 体系），若等级变化则发布 <see cref="MemberLevelChangedDomainEvent"/> 并记录变更历史。
    /// </summary>
    /// <param name="levels">已配置的成长值等级定义，按等级编号升序。</param>
    public void EvaluateGrowthLevel(IReadOnlyList<MemberLevel> levels)
    {
        ...
        AddDomainEvent(new MemberLevelChangedDomainEvent(UserId, oldLevel, newLevel, GrowthValue));
    }
