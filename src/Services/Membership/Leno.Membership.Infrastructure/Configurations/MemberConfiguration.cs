...
/// LevelChangeHistories 等级变更历史集合作为 owned collection 持久化到 member_level_change_histories 表。
/// </summary>
public sealed class MemberConfiguration : IEntityTypeConfiguration<MemberAggregate>
{
    public void Configure(EntityTypeBuilder<MemberAggregate> builder)
    {
        ...
        builder.Property(m => m.GrowthValue).HasColumnName("growth_value");
        builder.Property(m => m.GrowthValueUpdatedAt).HasColumnName("growth_value_updated_at");
        builder.Property(m => m.CurrentGrowthLevel).HasColumnName("current_growth_level");
