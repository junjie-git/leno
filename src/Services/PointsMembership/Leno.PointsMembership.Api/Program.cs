...
// 一站式注册：共享内核基础设施 + 鉴权 + 健康检查 + Controllers + OpenAPI + 积分会员域消费者 + 积分会员域基础设施 + 会员等级评估/积分过期后台服务
builder.Services.AddLenoApi<PointsMembershipDbContext>(
    builder.Configuration,
    "leno-points-api",
    cfg => cfg.AddPointsMembershipConsumers(),
    s =>
    {
        s.AddPointsMembershipInfrastructure(builder.Configuration);
        s.AddHostedService<Leno.PointsMembership.Api.BackgroundServices.MemberLevelEvaluationJob>();
        s.AddHostedService<Leno.PointsMembership.Api.BackgroundServices.PointsExpiryService>();
    });
