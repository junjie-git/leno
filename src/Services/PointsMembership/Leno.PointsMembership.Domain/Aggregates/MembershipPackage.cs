...
public sealed class MembershipPackage : AggregateRoot
{
    ...
    public void Enable()
    {
        ...
        {
            throw new PointsDomainException("套餐已启用", "PACKAGE_ALREADY_ENABLED");
        }
    ...
    public void Disable()
    {
        ...
        {
            throw new PointsDomainException("套餐已停用", "PACKAGE_ALREADY_DISABLED");
        }
    ...
    private static void Validate(string name, int level, decimal price, int durationDays, string benefits)
    {
        ...
        {
            throw new PointsDomainException("套餐名称不可为空", "PACKAGE_NAME_EMPTY");
        }

        if (level <= 0)
        {
            throw new PointsDomainException("套餐等级编号须大于 0", "PACKAGE_LEVEL_INVALID");
        }

        if (price <= 0)
        {
            throw new PointsDomainException("套餐价格须大于 0", "PACKAGE_PRICE_INVALID");
        }

        if (durationDays <= 0)
        {
            throw new PointsDomainException("套餐时长须大于 0", "PACKAGE_DURATION_INVALID");
        }

        if (string.IsNullOrWhiteSpace(benefits))
        {
            throw new PointsDomainException("套餐权益不可为空", "PACKAGE_BENEFITS_EMPTY");
        }
    }
}
