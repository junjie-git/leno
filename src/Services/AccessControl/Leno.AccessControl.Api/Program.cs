using Leno.AccessControl.Api.GrpcServices;
using Leno.AccessControl.Infrastructure;
using Leno.AccessControl.Infrastructure.Dependencies;
...
builder.Host.UseLenoSerilog(builder.Configuration, "leno-access-control-api");
builder.AddLenoOpenTelemetry();
builder.AddConsulServiceRegistration("leno-access-control-api");

// 一站式注册：共享内核基础设施 + 鉴权 + 健康检查 + Controllers + OpenAPI + AccessControl BC 基础设施
builder.Services.AddLenoApi<AccessControlDbContext>(
    builder.Configuration,
    "leno-access-control-api",
    configureInfrastructure: s => s.AddAccessControlInfrastructure(builder.Configuration));
