...
/// 保留 OPENJSON 优化：GetRolesByPermissionAsync 使用 SQL Server OPENJSON 在数据库端解析 JSON 权限列并按 resourceKey 精确过滤，
/// 避免全表加载后内存反序列化过滤（原 UserAuth BC P1-4 问题）。
/// </summary>
public sealed class EfCorePermissionRepository : IPermissionRepository
{
    private readonly AccessControlDbContext _context;

    public EfCorePermissionRepository(AccessControlDbContext context)
    {
