...
using Leno.UserCenter.Infrastructure.Dependencies;

var builder = WebApplication.CreateBuilder(args);

// Serilog 结构化日志 + OpenTelemetry 分布式追踪 + Consul 服务自注册
builder.Host.UseLenoSerilog(builder.Configuration, "leno-user-center-api");
builder.AddLenoOpenTelemetry();
builder.AddConsulServiceRegistration("leno-user-center-api");

// 一站式注册：共享内核基础设施 + 鉴权 + 健康检查 + Controllers + OpenAPI + UserCenter 域基础设施
builder.Services.AddLenoApi<UserCenterDbContext>(
    builder.Configuration,
    "leno-user-center-api",
    configureInfrastructure: s => s.AddUserCenterInfrastructure(builder.Configuration));
...
await app.Services.MigrateWithLockAsync<UserCenterDbContext>();
app.Run();
