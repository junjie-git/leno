...
public sealed class Favorite : AggregateRoot
{
    ...
    public static Favorite Create(Guid id, Guid userId, Guid spuId, DateTime? favoritedAt = null)
    {
        if (id == Guid.Empty)
        {
            throw new UserCenterDomainException("收藏标识不可为空", "FAVORITE_ID_EMPTY");
        }

        if (userId == Guid.Empty)
        {
            throw new UserCenterDomainException("用户标识不可为空", "FAVORITE_USER_EMPTY");
        }

        if (spuId == Guid.Empty)
        {
            throw new UserCenterDomainException("商品 SPU 标识不可为空", "FAVORITE_SPU_EMPTY");
        }
