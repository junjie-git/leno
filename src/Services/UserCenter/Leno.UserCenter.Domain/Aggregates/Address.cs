...
public sealed partial class Address : AggregateRoot
{
        ...
        bool isDefault = false)
    {
        if (id == Guid.Empty)
        {
            throw new UserCenterDomainException("地址标识不可为空", "ADDRESS_ID_EMPTY");
        }

        if (userId == Guid.Empty)
        {
            throw new UserCenterDomainException("用户标识不可为空", "ADDRESS_USER_EMPTY");
        }
    ...
    public void SoftDelete()
    {
        ...
        {
            throw new UserCenterDomainException("地址已删除", "ADDRESS_ALREADY_DELETED");
        }
    ...
    private void EnsureActive()
    {
        ...
        {
            throw new UserCenterDomainException("仅 Active 状态的地址可修改", "ADDRESS_NOT_ACTIVE");
        }
    }
    ...
    private static void ValidateRecipientName(string recipientName)
    {
        ...
        {
            throw new UserCenterDomainException("收件人不可为空", "ADDRESS_RECIPIENT_EMPTY");
        }

        if (recipientName.Trim().Length is < 1 or > 32)
        {
            throw new UserCenterDomainException("收件人姓名长度须为 1-32 字符", "ADDRESS_RECIPIENT_LENGTH");
        }
    }
    ...
    private static void ValidatePhone(string phone)
    {
        ...
        {
            throw new UserCenterDomainException("收件人手机号不可为空", "ADDRESS_PHONE_EMPTY");
        }

        if (!ValidPhonePattern().IsMatch(phone.Trim()))
        {
            throw new UserCenterDomainException("收件人手机号须为 E.164 格式", "ADDRESS_PHONE_FORMAT");
        }
    }
    ...
    private static void ValidateRegion(string province, string city, string district)
    {
        ...
        {
            throw new UserCenterDomainException("省/直辖市不可为空", "ADDRESS_PROVINCE_EMPTY");
        }

        if (string.IsNullOrWhiteSpace(city))
        {
            throw new UserCenterDomainException("市不可为空", "ADDRESS_CITY_EMPTY");
        }

        if (string.IsNullOrWhiteSpace(district))
        {
            throw new UserCenterDomainException("区/县不可为空", "ADDRESS_DISTRICT_EMPTY");
        }
    }
    ...
    private static void ValidateTag(string? tag)
    {
        ...
        {
            throw new UserCenterDomainException($"地址标签长度不可超过 {MaxTagLength} 字符", "ADDRESS_TAG_LENGTH");
        }
    }
