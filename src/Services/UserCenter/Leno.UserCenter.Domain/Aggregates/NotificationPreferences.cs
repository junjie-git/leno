...
public sealed class NotificationPreferences : AggregateRoot
{
    ...
    public static NotificationPreferences Create(Guid id, Guid userId)
    {
        if (id == Guid.Empty)
        {
            throw new UserCenterDomainException("通知偏好标识不可为空", "NOTIFICATION_PREFERENCES_ID_EMPTY");
        }

        if (userId == Guid.Empty)
        {
            throw new UserCenterDomainException("用户标识不可为空", "NOTIFICATION_PREFERENCES_USER_EMPTY");
        }
