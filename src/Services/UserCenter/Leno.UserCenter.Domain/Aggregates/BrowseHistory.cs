...
public sealed class BrowseHistory : AggregateRoot
{
    ...
    public static BrowseHistory Create(Guid id, Guid userId, Guid spuId, Guid? skuId = null, DateTime? viewedAt = null)
    {
        if (id == Guid.Empty)
        {
            throw new UserCenterDomainException("浏览历史标识不可为空", "BROWSE_HISTORY_ID_EMPTY");
        }

        if (userId == Guid.Empty)
        {
            throw new UserCenterDomainException("用户标识不可为空", "BROWSE_HISTORY_USER_EMPTY");
        }

        if (spuId == Guid.Empty)
        {
            throw new UserCenterDomainException("商品 SPU 标识不可为空", "BROWSE_HISTORY_SPU_EMPTY");
        }

        if (skuId.HasValue && skuId.Value == Guid.Empty)
        {
            throw new UserCenterDomainException("商品 SKU 标识不可为空 GUID", "BROWSE_HISTORY_SKU_EMPTY");
        }
