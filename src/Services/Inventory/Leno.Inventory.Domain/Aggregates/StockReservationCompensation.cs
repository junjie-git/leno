...
public sealed class StockReservationCompensation : AggregateRoot
{
        ...
        CompensationOperationType operationType = CompensationOperationType.Release)
    {
        ...
        {
            throw new InventoryDomainException("OrderId 不可为空", "STOCK_COMPENSATION_ORDER_EMPTY");
        }

        if (skuId == Guid.Empty)
        {
            throw new InventoryDomainException("SkuId 不可为空", "STOCK_COMPENSATION_SKU_EMPTY");
        }

        if (quantity <= 0)
        {
            throw new InventoryDomainException("补偿数量须大于 0", "STOCK_COMPENSATION_QTY_INVALID");
        }
        ...
        {
            throw new InventoryDomainException("补偿操作类型无效", "STOCK_COMPENSATION_OP_TYPE_INVALID");
        }
