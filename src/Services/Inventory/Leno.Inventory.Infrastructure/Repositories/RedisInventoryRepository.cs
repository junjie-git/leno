...
/// 迁移自 Order BC 的 RedisInventoryRepository，更新命名空间与 InventoryDbContext 引用。
/// Redis Key 设计：
/// - inventory:stock:{skuId} — 可用库存（String）
/// - inventory:reserved:{skuId}:{orderId} — 单订单单 SKU 预占数量（String）
/// </summary>
...
return 1";
    ...
    /// <summary>
    /// Lua 脚本：确认扣减库存，删除预占 key（预占已转为真实扣减，不归还可用库存）。
    /// KEYS[2] = 预占 key
    /// 返回：1
    /// </summary>
...
return 1";
    ...
    public async Task ReturnDeductedAsync(Guid skuId, Guid orderId, int quantity, CancellationToken ct = default)
    {
        ...
        // 双写：加载聚合根，调用 Replenish 归还已扣减库存（基线上调）
        await PersistAggregateAsync(
            async () => await _stockReservationRepository.GetBySkuIdAsync(skuId, ct),
    ...
    internal static string BuildStockKey(Guid skuId) => $"inventory:stock:{skuId}";
    internal static string BuildReservedKey(Guid skuId, Guid orderId) => $"inventory:reserved:{skuId}:{orderId}";
}
