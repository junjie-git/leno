...
builder.Host.UseLenoSerilog(builder.Configuration, "leno-inventory-api");
builder.AddLenoOpenTelemetry();
builder.AddConsulServiceRegistration("leno-inventory-api");

// 一站式注册：共享内核基础设施 + 鉴权 + 健康检查 + Controllers + OpenAPI + 库存域消费者 + 库存域基础设施
builder.Services.AddLenoApi<InventoryDbContext>(
