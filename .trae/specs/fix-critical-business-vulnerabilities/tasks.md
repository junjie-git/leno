...
- [x] Task 23: 缓存 Pattern 失效性能优化
  - [x] SubTask 23.1: `InvalidatePatternAsync` 改 `UNLINK` + 分批 SCAN
- [x] Task 24: 测试占位收尾
  - [x] SubTask 24.1: 删除 `NewFeatureTests.cs` 空文件；重命名 `NewFeatureTests1-6.cs` 为具名测试文件
  - [x] SubTask 24.2: 补 ReviewAfterSales / SellerShop / SystemAdmin 三个空 `Application.Tests` 关键测试
  - [x] SubTask 24.3: CI（`.github/workflows/ci.yml`）集成 `scripts/check-placeholders.sh`，违反即阻止合并
- [x] Task 25: 文档同步
