...
/**
 * 横向柱状图组件（04-seller-ops 模块内自建）
 *
 * - y 轴类目自上而下（第 1 名在最上方），满足「按 GMV 降序」的排行展示
 * - 柱条右侧展示数值标签，可通过 valueFormatter 自定义格式（如金额）
 * - topHighlight 时第 1 名柱条高亮为绿色
 */
...
<template>
  <div class="seller-chart-bar" :style="{ height: `${height}px` }">
    <Spin v-if="loading" tip="加载中..." class="seller-chart-bar__spin" />
