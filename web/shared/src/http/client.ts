...
import { AUTH_STORE_ID, type PersistedAuthState } from '../auth/keys'

/**
 * 全局 axios 实例（四端统一版本，源自 @leno/shared 提取）
 *
 * - baseURL: `/api`（Vite dev proxy 转发到后端 5001）
 * - timeout: 15s
 * - 请求拦截器：鉴权 / Idempotency-Key（写操作）/ X-Trace-Id
 * - 响应拦截器：HTTP 层错误转换 → ApiResponse 解包 → 业务层错误转换
 *
 * 统一决策（2026-09-24，@leno/shared 提取时收敛三处历史分歧）：
 * - trace 头统一为 `X-Trace-Id`（与网关 TracingTransform 注入头同名；
 *   原 seller/system-admin 发送的 X-Request-Id 后端并不消费）
 * - 写操作幂等键覆盖 POST/PUT/PATCH/DELETE 全集
 *   （Payment 应用层真实读取 Idempotency-Key；seller/system-admin 此前缺失该保护）
 */
 ...
 * 存储键使用共享常量 AUTH_STORE_ID（= pinia persist 默认 key = store id）。
 */
function readTokenFromStorage(): string | null {
  try {
    const raw = localStorage.getItem(AUTH_STORE_ID)
    if (!raw) return null
...
client.interceptors.request.use((config: InternalAxiosRequestConfig) => {
  ...
  // 2. traceId（X-Trace-Id），便于后端日志关联
  if (!config.headers.has('X-Trace-Id')) {
    config.headers.set('X-Trace-Id', generateIdempotencyKey())
  }
  // 3. 写操作（POST/PUT/PATCH/DELETE）自动携带 Idempotency-Key（显式传入时优先）
