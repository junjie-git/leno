...
- **二级菜单**：
  - 工作台：经营概览、销售趋势、库存预警
  - 商品管理：商品列表、商品编辑、SKU 管理、价格历史
  - 物流管理：运费模板、物流公司查询
  - 订单履约：待发货订单、全部订单、物流轨迹
  - 售后处理：售后列表、售后详情
  - 评价管理：评价回复
  - 店铺设置：入驻申请、店铺资料、资质管理、店铺前台预览
  - 报表导出：销售报表导出
  - 个人中心：登录、个人资料、通知中心
...
- **与共享设计系统的关系**：完全遵循 `shared/design-system.md`。主色 `#1677FF`、圆角 `6px`/`8px`、间距 4/8/12/16/24/32/48 体系、字体 PingFang SC 优先。组件库统一 Ant Design Vue 4.x（`a-` 前缀），不使用 Vant。差异点：无（完全遵循共享设计系统）。
