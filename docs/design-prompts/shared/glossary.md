...
| 卖家 | Seller | 卖家 | 卖家 | 商户、vendor、merchant |
...
- 「卖家」与「商家」均合法：「卖家」用于卖家后台自称场景（如「卖家中心」「卖家入驻」）；「商家」用于运营/系统管理后台作为业务对象他称（如「商家管理」「商家列表」「商家审核」）。同一页面内不混用
...
**字段名保留例外**：
- API 字段名 `refundNo` 在代码与 API 表格中保留英文，中文展示场景使用「退款编号」（不使用「退款单号」，因「退款单」属禁用同义词，易混淆）
- API 字段名 `orderNo` 中文展示用「订单编号」
- API 字段名 `shipmentNo` 中文展示用「履约单号」
