...
- **关键区域**：
  - 区域 A（当前会员状态条）：展示当前付费会员状态（如「付费会员年卡，有效期至 2027-07-26」）或「未开通付费会员」。
  - 区域 B（套餐卡片）：每张卡片展示套餐名称（如「付费会员年卡」）+ 价格（如「¥199/年」）+ 权益清单（折扣/积分倍数/免邮/专属客服/生日礼等）+ 订阅按钮；当前套餐标记「当前套餐」。
...
- **可访问性**：卡片 `role="article"`；按钮 `aria-label="订阅 付费会员年卡支付 199 元"`；对话框 `role="dialog"`。
